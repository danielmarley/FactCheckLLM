...
In jupyter notebooks, launch a console and execute `playwright install` first. Then run Added install_dependencies.ipynb to install all dependencies. Then you can run `python server.py` to serve the file on port 8080. You could also run `uvicorn server:app --reload --port 8080 --host 0.0.0.0` and that will refresh the server on any file changes.
